...
export const CanvasProperties = ({ 
  ...
  const updateGridBooleanValue = (key: 'gridEnabled' | 'snapToGrid', value: boolean) => {
    const newPreferences = { ...preferences };
    
    switch (key) {
      case 'gridEnabled':
        newPreferences.grid.enabled = value;
        break;
      case 'snapToGrid':
            ...
            >
              <div className="mt-3 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => updateGridBooleanValue('gridEnabled', !preferences.grid.enabled)}
                    className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors ${
                      preferences.grid.enabled ? 'bg-blue-600' : 'bg-gray-600'
                    }`}
                  >
                    <span
                      className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                        preferences.grid.enabled ? 'translate-x-4' : 'translate-x-0.5'
                      }`}
                    />
                  </button>
                </div>
