...
export const OptimizedThumbnail: React.FC<OptimizedThumbnailProps> = ({
  ...
  const handleImageError = useCallback(async () => {
    ...
    console.log(`🖼️ Thumbnail not available for label ${labelId}`);
...
export const useThumbnailGenerator = () => {
  const generateThumbnail = useCallback(async (
    ...
    try {
      ...
      if (result) {
        console.log(`✅ Generated thumbnail for label ${labelId} (${size})`);
        return result;
      }
      return null;
    } catch (error) {
      console.error(`❌ Failed to generate thumbnail for label ${labelId}:`, error);
      return null;
    }
  }, []);
  ...
  const batchGenerateThumbnails = useCallback(async (
    ...
    console.log(`📊 Batch thumbnail generation: ${successful}/${results.length} successful`);
